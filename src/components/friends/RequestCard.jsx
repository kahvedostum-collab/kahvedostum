  ...
  return (
    <div
      className={`group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border transition-all duration-300 hover:shadow-lg hover:scale-[1.01] ${
        isIncoming
          ? 'bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 border-green-100 dark:border-green-900/50 hover:border-green-200 dark:hover:border-green-700'
          : 'bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 border-amber-100 dark:border-amber-900/50 hover:border-amber-200 dark:hover:border-amber-700'
      }`}
    >
      <div className="flex items-center gap-4 mb-3 sm:mb-0">
        {/* Avatar */}
        <div className="relative">
          <Avatar className={`h-14 w-14 border-2 ring-2 transition-all duration-300 ${
            isIncoming
            ...
            <AvatarFallback className={`font-bold text-lg ${
              isIncoming
                ? 'bg-gradient-to-br from-green-100 to-emerald-100 dark:from-green-900/50 dark:to-emerald-900/50 text-green-800 dark:text-green-200'
                : 'bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/50 dark:to-orange-900/50 text-amber-800 dark:text-amber-200'
            }`}>
              {getInitials(user?.displayName || user?.userName)}
            </AvatarFallback>
          </Avatar>
          {/* Request type indicator */}
          <span className={`absolute -bottom-1 -right-1 h-6 w-6 rounded-full border-2 border-white flex items-center justify-center ${
            isIncoming
              ? 'bg-green-500'
              : 'bg-amber-500'
          }`}>
        ...
        <div className="flex flex-col">
          <p className={`font-semibold text-lg ${
            isIncoming ? 'text-green-900 dark:text-green-100' : 'text-amber-900 dark:text-amber-100'
          }`}>
            {user?.displayName || user?.userName || t('friends.card.unknownUser')}
          </p>
          {user?.userName && (
            <p className={`text-sm ${isIncoming ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400'}`}>
              @{user.userName}
            </p>
          )}
          <div className="flex items-center gap-1.5 mt-1">
            <Clock className={`h-3.5 w-3.5 ${isIncoming ? 'text-green-400' : 'text-amber-400'}`} />
            <span className={`text-xs ${isIncoming ? 'text-green-500 dark:text-green-400' : 'text-amber-500 dark:text-amber-400'}`}>
